...
# Contador global de podas, usado para la experimentación
podas = 0
...
def back_tracking(energia, fila, col, costo_acum, mejor_costo):
    ...
    n = len(energia)
    m = len(energia[0])
    # Acumular la energía del píxel actual
    ...
    # Caso base
    # Si no se podó, esta solución mejora la mejor conocida
    if fila == n - 1:
    ...
    mejor_sub_res = []
     # Explorar los vecinos válidos de la fila siguiente
    i = -1
    while i <= 1:
        next_col = col + i
        if 0 <= next_col < m:
            costo, res_act = back_tracking(energia, fila + 1, next_col, costo_acum, mejor_costo)
            # Si el camino desde ese vecino es mejor, actualizar
            if costo < mejor_costo_actual:
                mejor_costo_actual = costo
                mejor_sub_res = res_act
        i += 1
    # Devolver el costo total desde esta posición junto con el camino correspondiente
...
def encontrar_seam_backtracking(energia):
    global podas
    m = len(energia[0])
    podas = 0 # Reiniciar contador de podas para esta ejecución
    mejor_costo = [inf]  # lista para simular paso por referencia
    mejor_res = []
    mejor_total = inf

    # Probar cada columna de la primera fila como punto inicial
    for j in range(m):
        costo, res = back_tracking(energia, 0, j, 0, mejor_costo)
        # Si el seam encontrado desde esta columna es mejor, actualizar
